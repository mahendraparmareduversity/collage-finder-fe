...
export default function CollegeDetailContent({ college }: { college: CollegeDetail }) {
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 sm:py-8 min-w-0">
        ...
        <div className="space-y-8">
          <section className="bg-surface rounded-2xl border border-neutral-border p-6 shadow-sm">
            <h2 className="font-heading font-semibold text-lg text-neutral-text mb-4">Quick info</h2>
            ...
            <div className="mt-6 flex flex-col sm:flex-row gap-3">
              <Link
                href="/#cta"
                className="w-full sm:w-auto bg-cta hover:bg-cta-hover text-white py-3 px-6 rounded-btn font-bold text-sm transition-all text-center"
              >
          ...
          {(college.courses?.length > 0 || college.courseFees?.length) && (
            <section className="bg-surface rounded-2xl border border-neutral-border p-6 shadow-sm">
              <h2 className="font-heading font-semibold text-lg text-neutral-text mb-4">Fees as per course</h2>
              {college.courseFees?.length ? (
                <div className="overflow-x-auto min-w-0">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-neutral-border">
                        <th className="text-left py-2 pr-4 text-neutral-muted font-semibold uppercase tracking-wide">Course</th>
                        <th className="text-left py-2 text-neutral-muted font-semibold uppercase tracking-wide">Fee</th>
                      </tr>
                    </thead>
                    <tbody>
                      {college.courseFees.map((row, i) => (
                ...
                <ul className="space-y-2 text-sm">
                  {college.courses!.map((course) => (
                    <li key={course} className="flex justify-between items-center py-2 border-b border-neutral-border last:border-0">
          ...
          {showAbout && (
            <section className="bg-surface rounded-2xl border border-neutral-border p-6 shadow-sm min-w-0 overflow-hidden">
              <h2 className="font-heading font-semibold text-lg text-neutral-text mb-3">About</h2>
              <p className="text-neutral-muted leading-relaxed break-words">{college.description}</p>
            </section>
          )}

          {college.highlights && college.highlights.length > 0 && (
            <section className="bg-surface rounded-2xl border border-neutral-border p-6 shadow-sm min-w-0 overflow-hidden">
              <h2 className="font-heading font-semibold text-lg text-neutral-text mb-3">Highlights</h2>
          ...
          {showEligibility && (
            <section className="bg-surface rounded-2xl border border-neutral-border p-6 shadow-sm min-w-0 overflow-hidden">
              <h2 className="font-heading font-semibold text-lg text-neutral-text mb-3">Eligibility</h2>
              <p className="text-neutral-muted leading-relaxed break-words">{college.eligibility}</p>
            </section>
          )}

          {college.facilities && college.facilities.length > 0 && (
            <section className="bg-surface rounded-2xl border border-neutral-border p-6 shadow-sm">
              <h2 className="font-heading font-semibold text-lg text-neutral-text mb-3">Facilities</h2>
              <div className="flex flex-wrap gap-2">
                {college.facilities.map((f) => (
                  <span key={f} className="px-3 py-1.5 rounded-lg bg-neutral-bg border border-neutral-border text-sm text-neutral-text">
                    {f}
                  </span>
                ))}
              </div>
          ...
          {(showWebsite || college.phone || college.email || college.address) && (
            <section className="bg-surface rounded-2xl border border-neutral-border p-6 shadow-sm min-w-0 overflow-hidden">
              <h2 className="font-heading font-semibold text-lg text-neutral-text mb-4">Contact</h2>
              <ul className="space-y-3 text-sm text-neutral-muted">
                {showWebsite && (
                  <li className="min-w-0 overflow-hidden">
                    <a href={college.website!} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:text-cta break-all">
                ...
                {college.phone && (
                  <li className="flex items-center gap-2">
                    ...
                    <a href={`tel:${college.phone}`} className="hover:text-cta">{college.phone}</a>
                  </li>
                )}
                {college.email && (
                  <li className="flex items-center gap-2 min-w-0 overflow-hidden">
                    <Mail className="w-4 h-4 shrink-0" />
                    <a href={`mailto:${college.email}`} className="hover:text-cta break-all">{college.email}</a>
                  </li>
                )}
                {(college.address || college.pinCode) && (
                  <li className="flex items-start gap-2 min-w-0 overflow-hidden">
                    <MapPin className="w-4 h-4 shrink-0 mt-0.5" />
                    <span className="min-w-0 break-words">{[college.address, college.pinCode].filter(Boolean).join(', ')}</span>
                  </li>
                )}
              </ul>
            </section>
